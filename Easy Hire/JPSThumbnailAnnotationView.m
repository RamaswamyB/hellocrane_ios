...
@property (nonatomic, strong) EHHTTPClient *client;
...
- (void)setupImageView {
    ...
    _indicator.frame = CGRectMake(37, 36, _indicator.frame.size.width, _indicator.frame.size.height);
...
- (void)setupSubtitleLabel {
    _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-32.0f, 31.0f, 168.0f, 28.0f)];
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
  ...
  //  self.imageView.image = thumbnail.image;
    ...
    [self getThumnailImageForURL:thumbnail.vehicleImageURL];
}
...
- (void)getThumnailImageForURL: (NSString *)url
{
    if (_client) {
        [_client stop];
        _client = nil;
    ...
    UIImage *cachedImage = [[EHCache sharedCacheInstance]cachedImageForKey:[NSURL URLWithString:url] forCacheType:kCacheTypeMemory];
    ...
    else
    {
        ...
        _client = [EHHTTPClient connectionWithImageURL:[NSURL URLWithString:url] method:@"GET" paramBlock:^(void){
            ...
            if (self) {
                ...
                [[EHCache sharedCacheInstance] cacheImage:data forKey:[NSURL URLWithString:url] toCacheType:kCacheTypeMemory];
